> # ⚠️ DEPRECATED — historical reference only (archived 2026-06-16)
> This is the **original v1 implementation plan**, preserved verbatim for provenance. It is **out of
> date** and no longer describes the system as built. The Phase 0–5 foundation here shipped, but the
> architecture then diverged in two major ways the v1 plan never anticipated:
> 1. **The brain split out of process** — cognition, tools, and the 3-tier safety model
>    (v1 Phase 3/3.5 `tools.py` + `tests/test_safety.py`, neither of which exists in this repo
>    anymore) moved to a separate **gabagent** process over an HTTP/SSE contract. voice-agent is now
>    a *pluggable voice shell* that owns audio and delegates cognition to a swappable brain.
> 2. **An entire wake / duck / half-duplex subsystem was added** (wake word "Hey Aria", media
>    ducking, half-duplex turn-taking, interrupt-word barge-in, TTS leveling) — none of which is in
>    this document. The v1 "open VAD barge-in + verbal sleep/wake" activation model was *inverted*
>    by the un-AEC'd mic self-tripping on Aria's own voice.
>
> **For the current, reconciled architecture see [`../PLAN.md`](../PLAN.md).** Open roadmap/bugs live
> in `../TRACKER.md`. Read those, not this. This file is kept only so the original reasoning, machine
> facts, and Phase 0–5 verification steps remain recoverable.
